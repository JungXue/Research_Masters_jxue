...
days = c("2006-01-01","2006-01-02","2006-01-03")
  Leaf = c("AA", "BA" ,"AB")
   Number = c(12,5,34)
   
   data.frame(c("2006-01-01","2006-01-01","2006-01-01"))

which(names(anomaly.df) == Leaf)


Number = as.data.frame(matrix(c(0,2,0,
                                4,6,0,
                                4,6,0),nrow=length(days)))
  rownames(Number) = days
  colnames(Number) = Leaf
  Number["2006-01-02",AA]


   
for (i in days){
  for (j in Leaf){
    anomaly.df[i,j] = Number[i,j]
  }
}
head(anomaly.df)
...
if(any(anomaly.df[1:nrow(anomaly.df),2:ncol(anomaly.df)]<0))
  {}
  else{anomaly.df=anomaly.df}

### Function 8 ###
### point Anomalies  number(raw or %),  Date, Which leaf, 
#at raw data level
# Add      
# reduce
#at count data level
...
### Function 9 ###
### period anomalies   number(raw or %),  period, Which leaf, 
...
anomaly.test1 <- function(){
  ...
  point.anomaly <- function(data, anomalyCount = 99, day = "2006/01/01", whichroot=c("A","A")){
    ...
    whichroot[1]
    return(pointanomaly.df)
  }
  
  point.anomaly(raw1.df)
  
  
  
  period.anomaly <- function(data, start.day = NA,end.day = NA, 
                             anomalyCount = 99, distribution = "Normal"){
    distribution = 
      data[,] = data[,]+anomalyCount
    return(periodanomaly.df)
  }
}
#####################################################################################################
